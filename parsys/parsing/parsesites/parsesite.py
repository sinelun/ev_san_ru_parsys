...
class ParseSite(Grabber):
    ...
    def __init__(self, **kwargs):
        ...
        self.site_url = kwargs['site'].rstrip('/') if 'site' in kwargs else None
        ...
        self.parsing, created = Parsing.objects.get_or_create(id=self.parsing, type='sites', completed=False)
        # текущий сайт
        ...
        print(f'Parsing «{self.parsing}» was started...')

    def save_urls(self, urls, parent_level=None):
        """ Сохраняет в SitePage новые URL-ы, обработанные check_url(url),
            создаёт новые записи в SitePageParsing.
        """
        print('save_urls()', parent_level, urls)
    ...
    def save_to_db(self):
        """Сохраняет данные парсинга продукта в таблицу модели SiteData todo"""
